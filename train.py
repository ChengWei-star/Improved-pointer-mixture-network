...
try:
    from torch.utils.tensorboard import SummaryWriter  # tensorboard是可视化工具，可以绘制loss，acc等走势图像
except:
...
def train(config):
    writer = SummaryWriter('logs/' + config.name)  # SummaryWriter记录日志
    ...
    for epoch in range(start_epoch, config.train.epochs):
        ...
        for i, (n, t, p) in enumerate(tqdm(train_loader)):
            ...
            if (i + 1) % 100 == 0:
                ...
                writer.add_scalar('train/loss', loss.item(), epoch * total + i)   # train/loss 绘制图表
                writer.add_scalar('train/acc', acc_item, epoch * total + i)       # train/acc 绘制图表
...
#                 break
        ...
        if (epoch + 1) % config.train.eval_period == 0:
            with torch.no_grad():    # 不计算梯度，不双向计算
                model = model.eval()
                acc = 0.
                loss_eval = 0.
                ...
                writer.add_scalar('val/loss', loss_eval, epoch)   # 绘制图表
                writer.add_scalar('val/acc', acc, epoch)          # 绘制图表
