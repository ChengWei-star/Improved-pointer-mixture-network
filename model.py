...
class DecoderSimple(nn.Module):
    def __init__(
        ...
        self.embeddingN = nn.Embedding(vocab_sizeN, embedding_sizeN, vocab_sizeN - 1) # torch.nn.Embedding(numembeddings,embeddingdim)的意思是创建一个词嵌入模型
        self.embeddingT = nn.Embedding(vocab_sizeT + 3, embedding_sizeT, vocab_sizeT - 1) # numembeddings代表一共有多少个词, embedding_dim代表你想要为每个词创建一个多少维的向量来表示它
        ...
        self.w_global = nn.Linear(hidden_size * 3, vocab_sizeT + 3) # map into T   用于设置网络中的全连接层
    ...
    def embedded_dropout(self, embed, words, scale=None):
        ...
        X = F.embedding(words, masked_embed_weight,         # input参数是我们想要用向量表示的对象的索引，weight储存了我们的向量表示，这个函数的目的是输出一个索引和向量的对应关系.
    ...
    def forward(
        ...
        input = torch.cat([n_input, t_input], 1)   # 1 表示按列拼接
