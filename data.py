...
def data_gen(data, split_size):
    for sample in data:
        ...
        accum_p = []
        start_i = 0
        for i, item in enumerate(zip(*sample)):  # 将多个可迭代的对象作为参数，按照相同的index，分别将他们内部的元素取出，然后重新组合成一个新的tuple
            n, t, p = item                       # zip理解为数据重组
...
class MainDataset(torch.utils.data.Dataset):
    def __init__(self,
            N_filename = './pickle_data/JS_non_terminal_small.pickle',
    ...
    def collate_fn(self, samples, device='cpu'):
        ...
        for sn, st, sp in zip(sent_N, sent_T, sent_P):
            sn_tensor = torch.ones(
                s_max_length
                , dtype=torch.long    # dtype：the desired data type of returned tensor
                , device=device
            ) * self.eof_N_id
