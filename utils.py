...
class LabelSmoothingLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        """
        one_hot = torch.full((output.shape[1],), self.smoothing_value).to(self.device)  # torch.full(size, fill_value）：用指定的值去填充size
        one_hot[self.ignore_index] = 0
        model_prob = one_hot.repeat(target.size(0), 1)
        model_prob.scatter_(1, target.unsqueeze(1), self.confidence)
        model_prob.masked_fill_((target == self.ignore_index).unsqueeze(1), 0)

        return F.kl_div(output, model_prob, reduction='sum')   # 计算kl散度 F.kl_div() 衡量两个 概率分布 之间的不同程度
    
def accuracy(out, target, ignored_index, unk_index):
    out_ = np.array(out[target != ignored_index])        # 生成数组
